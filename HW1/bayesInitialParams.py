...
hw1_path = os.path.join(os.path.expanduser("~"), f'Documents\\Files\\AutoML\\HW1\\HW1')

def getDatasetResults(dataset, algorithm):
    path = os.path.join(hw1_path, f'results\\{algorithm}\\{dataset}.csv')
...
def getBayesInitialParams(datasets, algorithm):
    
    results = getDatasetResults(datasets[0], algorithm)

    for dataset in datasets[1:]:
        results = pd.merge(results, getDatasetResults(dataset, algorithm), on='params', how='outer')
...
if __name__ == '__main__':
    ...
    for algorithm in algorithms:
        print(f'{algorithm}: {getBayesInitialParams(datasets, algorithm)}')
